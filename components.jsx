...
function _potCharger(wallet, token) {
  ...
  _potEnVol = true;
  fetch(window.FA_API_URL + "/wallet/fb-earned", { headers: { Authorization: "Bearer " + token } })
...
function PotLigne({ etat, s = 12 }) {
  const r = etat && window.FA_POT ? window.FA_POT.resume(etat) : null;
  if (!r) return null;
  const l = window.FA_POT.ligne(r);
  return (
    <div className="flex between center pot-ligne" style={{ gap: 8 }}>
      <span className="mono" style={{ fontSize: s, color: "var(--text-dim)" }}>{I18N.t("BB_POOL_KIND_POT")}</span>
      <span className="mono" style={{ fontSize: s, color: l.couleur, textAlign: "right" }}>
        {I18N.t(l.cle, ...l.args)}
      </span>
    </div>
  );
...
function CoreIcon({ type, rarity, size }) {
  const s = size || 28;
  const [, force] = useState(0);
  useEffect(() => {
    const onReady = (e) => { if (!e.detail || e.detail.type === type) force((n) => n + 1); };
...
function LpBadge({ tier, fa, size = 22, flat = false }) {
  ...
  return <img className="lp-badge" src="assets/LOGO_cut.webp" alt="" title={tip} aria-label={tip}
