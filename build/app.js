...
(function instrumenteFetch() {
  ...
  const note = evenement => {
    ...
    etatEchecsApi = maj ? maj(etatEchecsApi, evenement, maintenant) : evenement === "ok" ? {
      n: 0,
      dernier: maintenant
    } : {
      n: etatEchecsApi.n + 1,
...
function freshState() {
  return {
    ...
    campaignProgress: {},
    campaignWeekly: {},
    // { "w-9": lastClearMs } — défi hebdo des boss (serveur)
    campaignTitles: [],
    ...
    playerTitle: "",
    lpTier: null,
    // palier Liquidity Guardian ("G1"/"G2"/null) — server-owned
    lpFa: null,
    // montant LP connu (POST /lp/refresh), null tant que non vérifié
    ordinalName: "",
    holderDays: 0,
    options: {
      sound: true,
...
function App() {
  ...
  }, [g.liquid, g.locked, g.roster, g.freeFights, g.totalFights, g.ticketsSilver, g.ticketsGold, g.session.wins, g.session.losses, g.playerName, g.ordinalName, g.playerTitle, g.lpTier, g.lang, g.authToken]);
  ...
  useEffect(() => {
    ...
    return () => {
      window.FA_ECHECS_API_ABONNE = null;
    };
  }, []);
  useEffect(() => {
  ...
  const actions = useMemo(() => ({
        ...
        if (saveResp.ok) {
          ...
          setG(s => {
            ...
            if (boostsData) {
              ...
              next.boosts = bm.charges;
              next.boostsArmed = bm.armed;
            }
    ...
    resolveFight({
      ...
      if (addr) {
        fetch(`${API_URL}/boosts/status/${addr}`).then(r => r.json()).then(bd => {
          const bm = mapBoostStatus(bd);
          setG(st => ({
            ...st,
            boosts: bm.charges,
            boostsArmed: bm.armed
          }));
        }).catch(() => {});
      }
    ...
    async toggleBoost(key, armed) {
      ...
      try {
        ...
        const data = await resp.json();
        if (data.status !== "ok") {
          const reason = data.error === "verification_requise" ? I18N.t("BO_NEED_VERIFIED") : data.error === "no_charges" ? I18N.t("BO_NO_CHARGES") : data.error || "Erreur serveur";
          return {
            ok: false,
            reason
          };
        ...
        setG(st => ({
          ...
          boosts: {
            ...st.boosts,
            [key]: data.charges ?? st.boosts[key]
          },
          boostsArmed: {
            ...st.boostsArmed,
            [key]: data.armed === true
    ...
    async pushUiState(patch) {
      ...
      try {
        ...
        const d = await r.json();
        if (d && d.ui_state && typeof d.ui_state === "object") setG(st => ({
          ...st,
          uiState: d.ui_state
        }));
        return {
    ...
    championPickBorrow(entry) {
      setG(st => {
        ...
        return {
          ...
          selected: st.selected.slice(0, cap)
        };
      });
    },
    championClearBorrow() {
