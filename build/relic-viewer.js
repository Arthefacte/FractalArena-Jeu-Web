/* Généré par tools/precompile.mjs depuis relic-viewer.jsx — NE PAS ÉDITER. */
(function () {
// relic-viewer.jsx — viewer 3D interactif d'une relique (rotation auto + drag), dispose GPU strict.
function RelicViewer({
  type,
  rarity,
  size
}) {
  ...
  React.useEffect(() => {
    ...
    function geoFor(t) {
      switch (t) {
        ...
        case "sapphire_plate":
          return new THREE.BoxGeometry(1.3, 1.3, 0.55);
        case "quartz_lens":
          return new THREE.IcosahedronGeometry(1);
        case "amber_cell":
          return new THREE.DodecahedronGeometry(1);
        case "cobalt_spring":
          return new THREE.TorusGeometry(0.75, 0.3, 14, 28);
        case "onyx_membrane":
          return new THREE.TetrahedronGeometry(1.25);
        case "jade_circuit":
          return new THREE.TorusKnotGeometry(0.6, 0.22, 80, 10);
        case "prism_matrix":
          return new THREE.ConeGeometry(1, 1.7, 6);
        default:
          return new THREE.OctahedronGeometry(1);
      }
    }
    ...
    function ensure() {
      if (window.FA_RELIC_MODELS.isReady(type)) {
        mountObj();
        return;
      }
      window.FA_RELIC_MODELS.loadModel(type).then(() => {
        if (disposed || obj) return;
        if (window.FA_RELIC_MODELS.isReady(type)) mountObj();else mountPrimitive();
      });
    }
    ensure();
    function loop() {
    ...
    const onMove = e => {
      if (dragging && obj) obj.rotation.y += (e.clientX - lastX) * 0.01, lastX = e.clientX;
    };
    const onUp = () => {
      dragging = false;
    };
