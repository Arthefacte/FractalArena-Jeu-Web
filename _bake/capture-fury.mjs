// capture-fury.mjs — one-off : rend le Fury Core (assets/cores/fury_core.glb) en rotation,
// capture 24 frames PNG → _bake/frames/fury_XX.png, pour assembler un GIF (ffmpeg).
// Usage : NODE_PATH="$(npm root -g)" node _bake/capture-fury.mjs
...
console.log("ouverture spin-fury.html…");
await page.goto(`http://127.0.0.1:${port}/_bake/spin-fury.html`, { waitUntil: "load", timeout: 120000 });
...
for (let i = 0; i < N_FRAMES; i++) {
  ...
  await canvas.screenshot({ path: path.join(OUT, `fury_${String(i).padStart(2, "0")}.png`) });
}
