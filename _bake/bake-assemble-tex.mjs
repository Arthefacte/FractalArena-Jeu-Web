...
await page.goto(`http://localhost:${port}/_bake/bake-textured-parts.html`, { waitUntil: "load" });
...
fs.writeFileSync(path.join(ROOT, "_bake", "_assemble-tex-frames.json"), JSON.stringify({ F, CELL, frames }));
fs.writeFileSync(path.join(ROOT, "_bake", "_assemble-tex-last.webp"), Buffer.from(frames[F-1].split(",")[1], "base64"));
fs.writeFileSync(path.join(ROOT, "_bake", "_assemble-tex-mid.webp"), Buffer.from(frames[Math.floor(F*0.4)].split(",")[1], "base64"));
