...
fs.writeFileSync(path.join(ROOT, "_bake", "_assemble-frames.json"), JSON.stringify({ F, CELL, frames }));
fs.writeFileSync(path.join(ROOT, "_bake", "_assemble-last.webp"), Buffer.from(frames[F-1].split(",")[1], "base64"));
fs.writeFileSync(path.join(ROOT, "_bake", "_assemble-mid.webp"), Buffer.from(frames[Math.floor(F*0.4)].split(",")[1], "base64"));
