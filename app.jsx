...
const { Team, Fosse, Arene, Forge, Wallet, Boosts, Perso, Options, ChatFab, RoomFab, Leaderboard, Quests, Campaign, Tour, LoginGate, TutorialGate, Link, Cinematique, Market, LockedBanner, Expeditions, Referral } = window;
...
const refConsumed = () => { BOOT_REF_CODE = null; };
...
function dailyPatch(q) {
  return {
    freeFights: q.freeFights, freeResetTs: q.freeResetTs,
    loopSilverToday: q.loopSilverToday, loopGoldToday: q.loopGoldToday,
...
function loadState() {
  try {
    ...
    return Object.assign(freshState(), s, {
      ...
      selected: (Array.isArray(s.selected) ? s.selected : []).filter((id) =>
        (Array.isArray(s.roster) ? s.roster : []).some((b) => b && b.id === id)),
      ordinalName: "",  // sera écrasé par le nom serveur à la connexion (branche 200)
      serverFight: null, // un replay ne survit jamais au rechargement (blobs antérieurs à l'exclusion)
...
function App() {
  ...
  useEffect(() => {
    ...
    try { localStorage.setItem(SAVE_KEY, JSON.stringify({ ...g, authToken: "", serverFight: null })); } catch (e) { }
  ...
  useEffect(() => {
    ...
    if (q.changed) setG((s) => ({ ...s, ...dailyPatch(q) }));
  }, [g.wallet]);
  ...
  const reveillePools = useCallback(() => {
    ...
    if (gRef.current.serverFight !== null) { reveilDiffere.current = true; return; }
    emetReveil();
  }, []);
  ...
  const actions = useMemo(() => ({
    ...
    async fetchReferral() {
      ...
      try {
        ...
        if (!r.ok) return { ok: false, reason: r.status === 403 || r.status === 404 ? String(r.status) : "server" };
    ...
    async claimAirdropIfNew(addr, token, isNew) {
      ...
      try {
        ...
        const r = await fetch(`${API_URL}/claim-airdrop`, {
          ...
          body: JSON.stringify({ wallet: addr, ...refBody() }),
        });
        if (r.ok) { newPlayerRef.current = false; refConsumed(); }
    ...
    async authenticate(addr) {
      ...
      try {
        ...
        if (token) {
          ...
          gRef.current = { ...gRef.current, authToken: token };
          setG((s) => ({ ...s, authToken: token }));
        }
    ...
    resolveFight({ win, free, betTier, betAmount, fromLocked, isLoop }) {
      ...
      if (win) {
        ...
        const payoutMult = !free && gRef.current.lpTier
          ? (D.ECON.LP_PAYOUT_MULT[gRef.current.lpTier] || D.ECON.PAYOUT_MULT)
          : D.ECON.PAYOUT_MULT;
        const payout = (srv && Number.isFinite(srv.payout) && srv.payout > 0)
          ? srv.payout
          : Math.round(base * payoutMult);
        summary.payout = payout; summary.net = payout - betAmount;
    ...
    async relicFuse(relicIds) {
      ...
      try {
        const resp = await fetch(`${API_URL}/forge/relic-fuse`, {
          method: "POST", headers: { "Content-Type": "application/json", "Authorization": `Bearer ${s.authToken}` },
          body: JSON.stringify({ wallet: s.wallet, relic_ids: relicIds }),
        });
        const data = await resp.json();
    ...
    async equipDisenchant(itemId) {
      ...
      if (s.liquid + s.locked < D.DISENCHANT_FEE) return { ok: false, reason: I18N.t("INSUFFICIENT", s.liquid + s.locked, D.DISENCHANT_FEE) };
      try {
        const resp = await fetch(`${API_URL}/forge/equip-disenchant`, {
          method: "POST", headers: { "Content-Type": "application/json", "Authorization": `Bearer ${s.authToken}` },
          body: JSON.stringify({ wallet: s.wallet, item_id: itemId }),
        });
        const data = await resp.json();
    ...
    async coreEquip(beastId, coreId) {
      ...
      try {
        const resp = await fetch(`${API_URL}/forge/core-equip`, {
          method: "POST", headers: { "Content-Type": "application/json", "Authorization": `Bearer ${s.authToken}` },
          body: JSON.stringify({ wallet: s.wallet, beast_id: beastId, core_id: coreId }),
        });
        const data = await resp.json();
        if (data.status !== "ok") return { ok: false, reason: data.error || "Erreur serveur" };
        if (Array.isArray(data.creatures)) setG((st) => ({ ...st, roster: data.creatures }));
    ...
    async expeditionsStart({ destination, beast_ids, mode, duration_s, ticket }) {
      const s = gRef.current;
      if (!s.wallet || !s.authToken) return { ok: false, reason: "auth" };
      try {
        const resp = await fetch(`${API_URL}/expeditions/start`, {
          method: "POST",
          headers: { "Content-Type": "application/json", "Authorization": `Bearer ${s.authToken}` },
          body: JSON.stringify({ destination, beast_ids, mode, duration_s, ticket: ticket || null }),
        });
        if (resp.status === 401) {
          const re = await actions.authenticate(gRef.current.wallet);
          if (!re) { toast(I18N.t("AUTH_EXPIRED"), "bad"); return { ok: false, reason: "auth" }; }
    ...
    async claimQuest(questId) {
      ...
      try {
        ...
        const data = await resp.json();
        setG((st) => ({ ...st, locked: Number(data.new_locked ?? st.locked) }));
        return { ok: true, data };
      } catch (e) {
        return { ok: false, reason: "network" };
      }
    ...
    async marketRefresh(itemType) {
      ...
      try {
        ...
        setG((st) => ({ ...st, market: { listings: (j && j.listings) || [], mine, item_type: t } }));
    ...
    async marketList(item, price) {
      ...
      const body = { wallet: s.wallet, price };
      if (item && typeof item.core_id === "string") body.core_id = item.id;
      else body.relic_id = item && item.id;
  ...
  const VIEWS = { team: Team, fosse: Fosse, arene: Arene, campaign: Campaign, tour: Tour, expeditions: Expeditions, quests: Quests, forge: Forge, market: Market, wallet: Wallet, boosts: Boosts, perso: Perso, leaderboard: Leaderboard, options: Options, lien: Link, parrainage: Referral };
  ...
  return (
    <FA_Ctx.Provider value={ctx}>
      ...
      {/* key={g.wallet} : un changement de compte (claim de liaison quand on est
          déjà connecté) remonte le ChatFab — sinon il garde l'historique de
          l'ancien compte sous la clé du nouveau (audit web 2026-09-08, P2#14). */}
...
function ChipBadge({ src, alt }) {
  return (
    <img className="chip-badge" src={window.FA_ASSET_URL(src)} alt={alt || ""} width={56} height={56} />
  );
}
    ...
    <header className="hdr">
      ...
      <div className="flex gap8 center wrap" style={{ justifyContent: "flex-end" }}>
        <span key={"lq" + liquidPop.n} className={cx("chip", "liquid", liquidPop.n > 0 && "pop")}>
          <ChipBadge src="assets/fa-badge.webp" />
          {fmt(g.liquid)}
        ...
        {fbBal && fbBal.status === "ok" && (
          <span className="chip fb" title={I18N.t("FB_CHIP_TITLE")}>
            ...
            <ChipBadge src="assets/fb-badge.webp" alt="FB" />
    ...
    <div className="app-shell" style={{ minHeight: "100vh", display: "grid", placeItems: "center", position: "relative", zIndex: 1 }}>
      <div style={{ textAlign: "center", maxWidth: 540, padding: 28, position: "relative" }}>
        ...
        {/* Action secondaire : le joueur qui a deja un wallet garde son flux d'avant.
            Sur mobile sans provider, on redirige vers l'app UniSat (OPEN_IN_UNISAT) :
            son navigateur intégré injecte le provider UniSat et le flux redevient celui du
            desktop. Dans ce navigateur-là, hasWallet est vrai → branche du haut. */}
        {hasWallet ? (
          ...
          <>
            <button className="btn block" style={{ marginTop: 12 }} disabled={!!busy} onClick={OPEN_IN_UNISAT}>
              {I18N.t("OB_OPEN_UNISAT_BTN")}
            </button>
            <div className="muted mono" style={{ fontSize: 11, marginTop: 8 }}>{I18N.t("OB_OPEN_UNISAT_SUB")}</div>
          </>
        ) : (
