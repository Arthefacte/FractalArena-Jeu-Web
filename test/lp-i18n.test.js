...
// et le texte d'aide porte les VRAIS seuils (50k / 500k de LP VERROUILLÉE —
// design round 2 du 03/09, jamais les anciens 2k/10k/200k).
...
test("i18n : les seuils affichés sont 50k et 500k, et les titres sont les bons", () => {
  const bloc = (k) => {
    const idx = src.indexOf(k + ":");
  ...
  assert.match(bloc("LP_PANEL_HINT"), /500[\s,]000/, "seuil 3D 500 000 absent du hint");
  ...
  // Aucun ancien seuil (2k / 10k / 200k) ne doit survivre dans les textes LP.
  ...
  assert.ok(!/\b200[\s,]000\b/.test(bloc("LP_PANEL_HINT")), "ancien seuil 200 000 encore présent");
});
