...
function aplatirPoolsHumain(poolsByInstance) {
  ...
  return ["buyback", "burn", "pot"]
    .filter((k) => hum[k])
    .map((k) => Object.assign({}, hum[k], {
      tier: k,
...
function texteTape(I, it) {
  if (it.type === "rachat") {
    const base = it.tier === "burn"
      ? I.t("TAPE_BURN_K", bbFmt(it.montant))
      : it.tier === "pot"
        ? I.t("TAPE_POT_K")
        : typeof it.tier === "string"
          ? I.t("TAPE_RACHAT_K", bbFmt(it.montant))
          : I.t("TAPE_RACHAT", bbFmt(it.tier), bbFmt(it.montant));
    return base + " · " + ageTexte(I, it.age);
  }
  if (it.type === "entree") return typeof it.tier === "string"
    ? I.t("TAPE_ENTREE_K", bbFmt(it.montant), labelPool(I, it.tier))
    : I.t("TAPE_ENTREE", bbFmt(it.montant), bbFmt(it.tier));
  if (it.type === "pool") return typeof it.tier === "string"
    ? I.t("TAPE_POOL_K", labelPool(I, it.tier), it.pct)
    : I.t("TAPE_POOL", bbFmt(it.tier), it.pct);
    ...
    async function load() {
      ...
      if (rb && rb.buyback && rb.buyback.pools && !Array.isArray(rb.buyback.pools)) {
        ...
        const rachats = window.FA_TAPE
          ? window.FA_TAPE.rachatsDetectes(prevPools.current, flat, poolsPret.current)
          : {};
        prevPools.current = flat;
        poolsPret.current = true;
        setBb(Object.assign({}, rb.buyback, { pools: flat }));
...
Object.assign(window, {
  BuybackTicker, buybackFraction, aplatirPoolsHumain, labelPool,
