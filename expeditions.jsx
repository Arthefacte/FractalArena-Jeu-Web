...
function Expeditions() {
  ...
  async function launch() {
    ...
    setBusyCall(true);
    let r = await actions.expeditionsStart({ destination: selWorld, beast_ids: sel, mode, duration_s: durH * 3600, ticket });
    if (!r.ok && r.reason === "retry") r = await actions.expeditionsStart({ destination: selWorld, beast_ids: sel, mode, duration_s: durH * 3600, ticket });
    setBusyCall(false);
        ...
        <div className="exq-worlds">
          {XU.WORLDS.map((w) => {
            ...
            const onClick = s === "free"
              ? () => { setSelWorld(w.id); setSel([]); setDurH(8); setMode("prudente"); setTicket(null); setView("config"); }
              : s === "ready" ? () => claim(e)
  ...
  function renderConfig() {
    ...
    return (
      <div className="exq-col">
        ...
        <span className="eyebrow" style={{ fontSize: 10 }}>2 · {I18N.t("EXP_DURATION")}</span>
        <div className="exq-durbox">
          <div className="exq-durhead">
            <b className="exq-mono exq-durval">{durH} {I18N.t("EXP_HOURS")}</b>
            <span className="exq-mono exq-dim" style={{ fontSize: 10 }}>{I18N.t("EXP_BACK_AT", fmtClock(new Date(Date.now() + durH * 3600e3)))}</span>
          </div>
          <input type="range" className="exq-slider" min={XU.DURATION_MIN_H} max={XU.DURATION_MAX_H} step={1}
            value={durH} onChange={(e) => setDurH(parseInt(e.target.value, 10))}
            aria-label={I18N.t("EXP_DURATION")} />
          <div className="exq-durticks exq-mono">
            <span>{XU.DURATION_MIN_H}h</span><span>6h</span><span>{XU.DURATION_MAX_H}h</span>
          </div>
          <div className="exq-mono exq-durnote">{I18N.t("EXP_DURATION_NOTE")}</div>
        </div>
        ...
        <span className="eyebrow" style={{ fontSize: 10 }}>4 · {I18N.t("EXP_TICKET_STEP")}</span>
        <div className="exq-tickets">
          {[[null, "EXP_TICKET_NONE", "EXP_TICKET_NONE_SUB", "var(--text-dim)", 999],
            ["argent", "EXP_TICKET_SILVER", "EXP_TICKET_SILVER_SUB", "var(--elec)", silverLeft],
            ["or", "EXP_TICKET_GOLD", "EXP_TICKET_GOLD_SUB", "var(--gold)", goldLeft]].map(([k, tk, sk, accent, left]) => {
            const on = ticket === k;
            const out = left <= 0;
            return (
              <button key={String(k)} className="exq-toggle exq-ticket" disabled={out}
                onClick={() => { if (!out) setTicket(k); }}
                style={{ alignItems: "flex-start", background: on ? "rgba(255,230,0,.08)" : "var(--bg-panel)",
                  borderColor: on ? accent : "var(--line)", color: on ? accent : out ? "var(--text-faint)" : "var(--text)",
                  opacity: out ? 0.5 : 1, cursor: out ? "not-allowed" : "pointer" }}>
                <b style={{ fontSize: 12, letterSpacing: ".4px" }}>
                  {I18N.t(tk)}{k !== null && <span className="exq-mono" style={{ fontSize: 10, opacity: 0.7 }}> ×{left}</span>}
                </b>
                <span style={{ fontSize: 10, lineHeight: 1.35, color: "var(--text-dim)", textAlign: "left" }}>{I18N.t(sk)}</span>
              </button>
            );
        ...
        <div className="panel" style={{ padding: "13px 14px", border: "1px solid " + (ready3 ? successColor : "var(--line)") }}>
          ...
          {ready3 && ticket === "or" && (
            <div className="exq-mono exq-goldnote">{I18N.t("EXP_TICKET_GOLD_LOCKED")}</div>
          )}
          {ready3 && <div className="exq-mono" style={{ fontSize: 10, color: "var(--text-faint)", marginTop: 6 }}>{I18N.t("EXP_RATE_NOTE")}</div>}
          {ready3 && win && (
            <div className="exq-lootprev">
              <div className="exq-lootcol">
                <span className="exq-mono exq-cap">{I18N.t("EXP_LOOT_WIN")}</span>
              ...
              <div className="exq-lootcol">
                <span className="exq-mono exq-cap">{I18N.t("EXP_LOOT_LOSE")}</span>
                {lose.fa > 0
                  ? <b className="exq-mono" style={{ color: "var(--text-dim)" }}>+{lose.fa} FA</b>
          ...
          {allEpic && durH < XU.DUST_MIN_H && (
            <div className="exq-mono exq-dustnote">{I18N.t("EXP_DUST_NEEDS_8H")}</div>
          )}
        </div>
