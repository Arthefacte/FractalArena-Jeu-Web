...
function GuideBar() {
  ...
  const refresh = () => {
    actions.discoveryState().then((r) => { if (alive.current && r.ok) setDisc(r.data); }).catch(() => {});
  };
  ...
  useEffect(() => {
    const onProgress = (e) => {
      ...
      if (url.indexOf("/tower/fight") >= 0 || url.indexOf("/pvp/attack") >= 0) {
        setFlags((f) => {
          const next = { ...f, ...(url.indexOf("/tower/fight") >= 0 ? { towerPlayed: true } : {}), ...(url.indexOf("/pvp/attack") >= 0 ? { pvpPlayed: true } : {}) };
  ...
  const res = GU.computeGuide({ disc, g, flags, view: g.view });
  ...
  useEffect(() => {
    if (res.mode === "done" && disc && !hidden) {
      const t = setTimeout(() => { setHidden(true); lsSet(HIDDEN_KEY, true); }, 8000);
      return () => clearTimeout(t);
    }
    return undefined;
  }, [res.mode, disc, hidden]);

  const hide = () => { setHidden(true); lsSet(HIDDEN_KEY, true); };
