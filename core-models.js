// core-models.js — chargement + cache + instances teintées des modèles de cores.
import * as THREE from "three";
import { GLTFLoader } from "three/addons/loaders/GLTFLoader.js";
import { MeshoptDecoder } from "three/addons/libs/meshopt_decoder.module.js";

(function () {
  const A = window.FA_CORE_ASSETS;
  const loader = new GLTFLoader();
  // Les .glb des cores sont compressés en EXT_meshopt_compression (gltf-transform) :
    ...
    _loading[type] = new Promise((resolve) => {
      loader.load(
        ...
        (gltf) => {
          ...
          window.dispatchEvent(new CustomEvent("fa:core-model-ready", { detail: { type } }));
          resolve();
        },
        undefined,
  ...
  // Préchargement des écrans qui affichent des cores : utile pour éviter des
  // replis qui clignotent, mais jamais au prix d'un rendu en cours.
  ...
  // les zones prévues par l'artiste. Là où elle MANQUE, on ne repeint pas la
  // matière (même correctif que les reliques — voile blanc sur les modèles
  // sombres). makeInstance signale le cas (`tinted`) pour que l'appelant porte
  // la rareté autrement — la vignette le fait avec sa lumière d'appoint.
  function makeInstance(type, rarity) {
    if (!_loaded[type]) throw new Error("core model not ready: " + type);
  ...
  window.FA_CORE_MODELS = { preload, preloadWhenIdle, isReady, makeInstance, loadModel };
})();
