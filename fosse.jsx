...
function BoostPills() {
  ...
  async function tap(key) {
    ...
    if (!g.boostsArmed[key] && (g.boosts[key] || 0) <= 0) { toast(I18N.t("BO_NO_CHARGES"), "bad"); return; }
    setBusy(key);
    ...
    <div className="container wide">
      <div className="flex between center wrap" style={{ marginBottom: 14, gap: 12 }}>
        ...
        <div className="flex gap8 wrap">
          ...
          <span className="pill" style={{ color: "var(--gold)" }}>{I18N.t("AR_LOOPS_LEFT", Math.max(0, D.ECON.LOOP_SILVER_MAX - g.loopSilverToday), D.ECON.LOOP_SILVER_MAX, Math.max(0, D.ECON.LOOP_GOLD_MAX - g.loopGoldToday), D.ECON.LOOP_GOLD_MAX)}</span>
